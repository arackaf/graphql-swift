...
struct AllTagsFilters: Codable {
    ...
    var OR: Array<TagFilters?>?
    var SORT: TagSort?
    var SORTS: Array<TagSort?>?
    var LIMIT: Int?
    var SKIP: Int?
    var PAGE: Int?
    var PAGE_SIZE: Int?
...
func allTags(_ filters: AllTagsFilters, buildSelection: (TagQueryResultsBuilder) -> ()) throws -> (GenericGraphQLRequest<AllTagsFilters>, Codable.Type) {
    let selectionBuilder = TagQueryResultsBuilder()
    buildSelection(selectionBuilder)

    let selectionText = try selectionBuilder.emit()
    let query = getQueryText(selectionText);

    return (GenericGraphQLRequest<AllTagsFilters>(query: query, variables: filters), TagQueryResults.self)
}